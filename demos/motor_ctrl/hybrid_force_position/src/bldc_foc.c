...
#define MOTOR0_CURRENT_LOOP_BANDWIDTH (1000)
#define ADCU_INDEX 0
#define ADCV_INDEX 1
...
#endif
    mcl_hybrid_ctrl_cfg_t hybrid_ctrl_cfg;
    mcl_hybrid_ctrl_state_t hybrid_ctrl_state;
    struct {
        mcl_cfg_t mcl;
...
ATTR_PLACE_AT_FAST_RAM_INIT bool hybrid_ctrl_mode = false;
...
void motor_init(void)
{
    ...
    /**
     * @brief IIR lowpass filter: fpass 100Hz, fstop 2000Hz
     */
...
void isr_adc(void)
{
    ...
    if ((status & BOARD_BLDC_ADC_TRIG_FLAG) != 0) {
        ...
        if (hybrid_ctrl_mode) {
            hpm_mcl_encoder_get_absolute_theta(&motor0.encoder, &motor0.hybrid_ctrl_state.q_actual);

            motor0.hybrid_ctrl_state.dq_actual = hpm_mcl_encoder_get_speed(&motor0.encoder);

            mcl_hybrid_ctrl_step(&motor0.hybrid_ctrl_cfg, &motor0.hybrid_ctrl_state);
...
void motor_angle_align(void)
{
    ...
    alignment_cfg.config.three_stage.stage1.d_current = 2.0f;    /* High current for stage 1 */
...
int main(void)
{
    ...
    motor_angle_align();
    user_current.enable = true;
    user_current.value = 0.0f;
    hpm_mcl_loop_set_current_q(&motor0.loop, user_current);
    printf("Hybrid force-position control mode.\r\n");
    ...
    printf("Speed filter: alpha=%.3f, deadzone=%.2f rad/s\r\n",
            (double)motor0.hybrid_ctrl_cfg.speed_lpf_alpha,
            (double)motor0.hybrid_ctrl_cfg.speed_deadzone);
    printf("Target position: %d degrees\r\nInput position:\r\n", position);
    while (1) {
        ...
        if (i != 0) {
            ...
            mcl_hybrid_ctrl_set_position(&motor0.hybrid_ctrl_cfg, (float)position * 0.00157079632f);
            printf("Pos: %d deg, Tau: %.4f Nm, Pos_err: %.4f, Speed: %.3f rad/s\r\n",
                    position,
                    (double)motor0.hybrid_ctrl_state.tau_output,
                    (double)(motor0.hybrid_ctrl_state.pos_error),
                    (double)motor0.hybrid_ctrl_state.dq_actual);
